...
def main(input="iris",classifier="forest",uncertainty="entropy", extra_params={}):
    ...
    st.pyplot.xlim(game.X_raw[:,0].min()-0.5,game.X_raw[:,0].max()+0.5)
    st.pyplot.ylim(game.X_raw[:,1].min()-0.5,game.X_raw[:,1].max()+0.5)
    st.pyplot.show()
...
class game_state:
    def __init__(self, input="iris", clf="forest", unc="entropy", N_QUERIES=0, training_indices = None, show_acc = True, extra_params=None):
        global props
        ...
        if input == "iris":
            ...
            # This one is supposed to take the glass.data dataset and just choose a subset of classes to make it nicer to "play" with.
            # Unfortunately, this messes with the color scheme. This is just cosmetic, but should be fixed in the long run.
            X_rew = np.genfromtxt('glass.data', delimiter = ',', skip_header=0)[:,1:-1]
            y_names = np.genfromtxt('glass.data', dtype = str, delimiter = ',', skip_header=0)[:,-1]
            #Preprocess classes into numerical values:
            my_dict = {name : ix for ix, name in enumerate(np.unique(y_names))}
            ...
            X_rew, y_names = make_circles(n_samples=200, noise=0.05, factor=0.5, random_state=42)
            my_dict = {name: ix for ix, name in enumerate(np.unique(y_names))}
            y_rew = np.fromiter([my_dict[zi] for zi in y_names], dtype=int)
        elif input == "wine":
            wine = load_wine()
            X_rew = wine.data
            y_rew = wine.target
            labels = wine.target_names  # optional, for plot legend
        elif input == "digits":
            digits = load_digits()
            X_rew = digits.data[:300]  # Limit to 300 samples for performance
            y_names = digits.target[:300]
            # Preprocess classes into numerical values:
            my_dict = {name: ix for ix, name in enumerate(np.unique(y_names))}
            y_rew = np.fromiter([my_dict[zi] for zi in y_names], dtype=int)
        else:
    ...
    #     self.citizen_science_mode = True  # toggle on/off
    #     self.human_selected_points = []   # store indices of human clicks

    # def citizen_science_query(self):
    #     if not self.human_selected_points:
    #         print("[Citizen Science] No human-selected points yet.")
    #         return None
        
    #     selected_coords = self.X_raw[self.human_selected_points]
    #     print(f"[Citizen Science] Using {len(self.human_selected_points)} selected points for query.")
    
    # # Find point in pool most similar to selected ones
    #     distances = np.min(
    #         np.linalg.norm(self.X_pool[:, None] - selected_coords[None, :], axis=2),
    #         axis=1
    #     )
    #     idx = np.argmax(distances)  # or np.argmin for closest points
    #     print(f"[Citizen Science] Suggested next point index in pool: {idx}, coords: {self.X_pool[idx]}")
    
    #     # Highlight the suggested point visually
    #     self.hint_index = np.nonzero((list(self.points_dict.values()) == self.X_pool[idx]).all(axis=1))[0][0]
    #     self.drawn_points[self.hint_index][1].set_edgecolor("purple")
    #     self.drawn_points[self.hint_index][1].set_facecolor("yellow")
    #     self.arrow = self.ax.annotate(
    #         'Suggested',
    #         self.X_pool[idx],
    #         xytext=self.X_pool[idx] - np.array((.75, .75)),
    #         arrowprops=dict(facecolor='black', shrink=0.2),
    #     )
    #     plt.draw()
...
if __name__ == "__main__":
    ...
    # # Save selections if Citizen Science mode is on
    #     if game.citizen_science_mode:
    #         with open("citizen_science_log.json", "w") as f:
    #             json.dump({"selected_points": game.human_selected_points}, f)
    #             print("Citizen Science log saved to citizen_science_log.json")
    #     return  # or whatever your main returns
